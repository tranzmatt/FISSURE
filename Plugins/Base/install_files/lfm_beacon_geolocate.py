...
class OperationMain(Operation):
    ...
    def _apply_parameters_from_runner(self) -> None:
        ...
        freq_value = p.get("freq_mhz", self.freq_mhz)
        try:
            self.freq_mhz = float(freq_value)
        except Exception:
            self.freq_mhz = 433.0

        interval_value = p.get("min_detection_interval_s", self.min_detection_interval_s)
        try:
            self.min_detection_interval_s = float(interval_value)
        except Exception:
            self.min_detection_interval_s = 1.0

        self.description = str(p.get("description", self.description)).strip() or "LFM beacon geolocation"
    ...
    async def _emit_detection(
        ...
        *,
        ...
        await self.tak_cot_callback({
            "msg_type": "event",
            "uid": f"lfm-beacon-geolocate-{self.target_id}-{int(det_time)}",
            "lat": True,
            "lon": True,
            "alt": True,
            "time": True,
