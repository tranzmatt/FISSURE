...
First-pass IQ recorder operation for the Base plugin.

Assumptions:
  - Artifact files are local/unpacked under FISSURE/artifacts/<operation_id>/files.
  - B2x0/B20xmini flow graph files are copied into:
      Plugins/Base/install_files/iq_record_flow_graphs/<maint-version>/b2x0/
  - Zip transfer/download behavior is handled elsewhere later.
"""
...
class OperationMain(Operation):
    ...
    def __init__(
        ...
        sample_rate_mhz: Union[str, float] = 1.0,
        sample_rate: Union[str, float] = 1.0,
        ...
        self.base_file_name = os.path.basename(str(base_file_name or "capture.sigmf-data").strip())
        ...
        self.sample_rate_mhz = self._float(sample_rate_mhz, self._float(sample_rate, 1.0))
        self.sample_rate = self._float(sample_rate, self.sample_rate_mhz)
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            manifest_path = self._write_manifest(folder, recorded_files)

            metadata = {
                "role": "iq_recording_raw_v1" if self.artifact_format == "raw" else "iq_recording_bundle_v1",
                ...
                "sample_rate_mhz": self.sample_rate,
    ...
    async def _record_one_file(self, output_path: str) -> None:
        """
        Runs one generated GNU Radio recorder file and waits for it to finish.
        """
        ...
        try:
            ...
            await loop.run_in_executor(None, tb.wait)
    ...
    def _write_sigmf_meta(self, data_path: str, file_index: int) -> str:
        ...
        meta["global"]["core:sample_rate"] = float(self.sample_rate) * 1e6
        ...
        meta["captures"][0]["core:datetime"] = datetime.datetime.utcnow().isoformat("T") + "Z"
    ...
    def _write_manifest(self, folder: str, recorded_files) -> str:
        manifest = {
            "operation_id": self.operation_id,
            "node_uid": self.node_uid,
            "description": self.description,
