...
import fixed_threshold_b2x0_epy_block_0 as epy_block_0  # embedded python block
import numpy as np
import random
import sip
...
class fixed_threshold_b2x0(gr.top_block, Qt.QWidget):

    def __init__(self, antenna_default='TX/RX', channel_default='A:A', gain_default='60', ip_address='', min_interval="1", rx_freq_default='2412000000', sample_rate_default='20e6', serial='False', threshold_default='0'):
        ...
        self.settings = Qt.QSettings("GNU Radio", "fixed_threshold_b2x0")
        ...
        self._up_line_adj_range = qtgui.Range(1, 8191, 1, 8191, 200)
        self._up_line_adj_win = qtgui.RangeWidget(self._up_line_adj_range, self.set_up_line_adj, "'up_line_adj'", "counter_slider", int, QtCore.Qt.Horizontal)
        self.top_grid_layout.addWidget(self._up_line_adj_win, 5, 0, 1, 4)
        for r in range(5, 6):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._thresh_adj_range = qtgui.Range(-120, 20, 1, float(threshold_default), 200)
        self._thresh_adj_win = qtgui.RangeWidget(self._thresh_adj_range, self.set_thresh_adj, "Thresh", "counter_slider", float, QtCore.Qt.Horizontal)
        ...
        for c in range(0, 1):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._rx_gain_range = qtgui.Range(0, 80, 1, float(gain_default), 200)
        ...
        for c in range(0, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._rx_freq_range = qtgui.Range(50e6, 6000e6, .1e6, float(rx_freq_default), 200)
        ...
        for c in range(0, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        # Create the options list
        self._rx_antenna_options = ['TX/RX', 'RX2']
        # Create the labels list
        self._rx_antenna_labels = ['TX/RX', 'RX2']
        # Create the combo box
        ...
        for c in range(1, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._low_line_adj_range = qtgui.Range(1, 8191, 1, 1, 200)
        self._low_line_adj_win = qtgui.RangeWidget(self._low_line_adj_range, self.set_low_line_adj, "'low_line_adj'", "counter_slider", int, QtCore.Qt.Horizontal)
...
def argument_parser():
    ...
    parser.add_argument(
        "--gain-default", dest="gain_default", type=str, default='60',
